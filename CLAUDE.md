...
- **Recommendation order (how to regenerate).** Each word is scored by the sum of its **distinct** letters' frequency, sorted descending (alphabetical tie-break). Frequency = **presence counts over the 2,315-word answer list** (`AnswerList.ts`): for each letter, the number of answer words containing it at least once. The answer list is the population the hidden word is drawn from, so this maximizes information about the answer; general-English tables (e.g. Wikipedia's letter-frequency "Dictionaries" column, used before 2026-06-13) over-weight `s` because NYT answers avoid plural-`s` words. Letter rank over answers: `e a r o t l i s n u …`; top openers come out as `oater/orate/alert/alter/later/irate`. Keep this simple — do not switch to entropy / average-remaining-candidates metrics. Regeneration is a ~10-line node script: parse both files, score, sort, rewrite `WordList.ts` (and `wordlist.txt`, same order, plain words).
